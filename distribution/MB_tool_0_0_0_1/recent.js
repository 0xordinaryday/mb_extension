...
$(function() {
        ...
        chrome.storage.local.get({'MBFavList':['No names']}, function (items) {
            ...
            for(var i = 0; i < keyArr.length; i++) {
                ...
                if (name_array.indexOf(author_name) != -1) {
                    author_name = "<span style='color:red;font-weight:bold'>"+author_name+"</span>";
                } // highlight author
                var c_preview = properties["mySummary"];
                var c_link = properties["myLink"];
                longString = longString + "<a href="+'"'+c_link+'"'+"> Comment by: "+author_name+"</a><br>"+timeString+"<p>"+c_preview+"</p>";
            }
    ...
    function calc_time(timeval) {
        ...
        {
            return timeval + ' Seconds' + pluralise(timeval) + ' ago';
        }
    } 
